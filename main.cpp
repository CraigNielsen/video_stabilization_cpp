...
int main(int argc, char **argv)
{
    ...
    while(true) {
        ...
        for(size_t i=0; i < status.size(); i++) {
            if(status[i]) {
                prev_points2.push_back(prev_points[i]);
                cur_points2.push_back(cur_points[i]);
            }
        }

        // translation + rotation only
        Mat T = estimateRigidTransform(prev_points2, cur_points2, false); // false = rigid transform, no scaling/shearing
        ...
        cout << "Frame: " << k << "/" << max_frames << " - good optical flow: " << prev_points2.size() << endl;
        k++;
    }
    ...
    int frame_codec= cap.get(CV_CAP_PROP_FOURCC);
    VideoWriter video("/home/craig/out.MOV",frame_codec,frame_rate + 3, Size(frame_width,frame_height),true);
    k=0;
